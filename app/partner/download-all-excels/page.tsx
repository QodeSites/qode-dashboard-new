...
export default function PartnerDownloadAllExcelsPage() {
  ...
  useEffect(() => {
    ...
    if (status === "authenticated" && session?.user?.accessType !== "partner") router.push("/dashboard");
  ...
  async function handleDownload() {
    ...
    try {
      const res = await fetch("/api/partner/download-all-excels");
      ...
      a.download = `portfolio_excels_partner_book_${date}.zip`;
      ...
      <div className="max-w-xl mx-auto px-4 py-16 space-y-8">
        ...
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-card-text-secondary">
            Generates one Excel per strategy for every client in your book and returns a single ZIP file.
        ...
        <div className="bg-white/60 border border-card-text-secondary/20 rounded-2xl p-8 space-y-6">
          ...
          <p className="text-xs text-card-text-secondary text-center">
            Partner only · Files include all strategies for clients in your book · Benchmark columns will be blank
          </p>
        </div>
      </div>
    </div>
