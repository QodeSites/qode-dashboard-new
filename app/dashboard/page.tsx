...
export default function Portfolio() {
  ...
  const isSuresh = effectiveIcode === "QUS00086";
  const isBifurcatedClient = isDinesh || isShilpa || isVikram || isSuresh;
    ...
    if (status === "authenticated") {
      ...
      if (isSarla) {
        ...
        const bifurcatedConfig = isDinesh
          ...
          : { api: "/api/suresh-api", qcode: "QAC00072", name: "Suresh Somani" };
...
/>
    ...
    return (
      <div className="space-y-6">
        ...
        <StatsCards
          stats={convertedStats}
          accountType="sarla"
          broker={isDinesh ? "Dinesh" : isShilpa ? "Shilpa" : isVikram ? "Vikram Trading" : "Suresh Somani"}
          isTotalPortfolio={isTotalPortfolio}
          isActive={isActive}
          returnViewType={returnViewType}
          setReturnViewType={setReturnViewType}
  ...
  if ((isSarla || isSatidham || isBifurcatedClient) && (!sarlaData || availableStrategies.length === 0)) {
    return (
      <div className="p-6 text-center bg-[#f3f4f6] rounded-lg text-card-text">
        No strategy data found for {isSarla ? "Sarla" : isSatidham ? "Satidham" : isDinesh ? "Dinesh" : isShilpa ? "Shilpa" : isVikram ? "Vikram Trading" : "Suresh Somani"} user.
      </div>
    );
  }
