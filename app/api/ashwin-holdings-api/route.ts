...
const ASHWIN_ICODE = "QUS00097";
const ASHWIN_QCODE = "QAC00083";
...
export async function GET() {
  try {
    ...
    if (icode !== ASHWIN_ICODE) return NextResponse.json({ error: "Forbidden" }, { status: 403 });

    const latestEquity = await prisma.bifurcated_equity_holding_test.findFirst({
      where: { qcode: ASHWIN_QCODE },
      orderBy: { date: "desc" },
      select: { date: true },
    });
    ...
    const equityRows = latestEquity
      ? await prisma.bifurcated_equity_holding_test.findMany({
          where: { qcode: ASHWIN_QCODE, date: latestEquity.date },
        })
      : [];
    const mfRows = latestMf
      ? await prisma.bifurcated_mutual_fund_holding_sheet_test.findMany({
          where: { qcode: ASHWIN_QCODE, as_of_date: latestMf.as_of_date },
        })
      : [];
    ...
    console.error("Ashwin holdings API error:", error);
    return NextResponse.json(
